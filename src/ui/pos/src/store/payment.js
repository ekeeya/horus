...
export const getCard =   createAsyncThunk("payment/getCard",async (cardNo, thunkAPI) => {
    try{
...
export const getTransactions =   createAsyncThunk("payment/getTransactions",async (id, thunkAPI) => {
    try{
...
export const makePayment = createAsyncThunk("payment/makePayment",async (data, thunkAPI) => {
    try{
        const response = await client.post(`/api/v1/wallet/make-payment`,data)
        return response.data.data
    }catch (error){
        const errorMsg = generateError(error)
        return thunkAPI.rejectWithValue(errorMsg)
...
export const { resetCardDetails, setPaymentAmount, setPaid,resetError } = paymentSlice.actions
