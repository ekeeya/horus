...
import {columns} from "@src/views/apps/inventory/orders/columns";
...
import {getOrders} from "@src/views/apps/inventory/store";
...
const Orders = (props) => {
  ...
  const {loading, orders, pages} =  useSelector(store => store.inventory)
  ...
  useEffect(() => {
    dispatch(getOrders(filterPayLoad))
